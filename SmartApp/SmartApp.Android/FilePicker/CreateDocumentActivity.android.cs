...
namespace SmartApp.FilePicker
{
    ...
    {
        /// <summary>
        /// Intent Extra constant to pass list of allowed types to CreateDocument activity.
        /// </summary>
        public const string ExtraDocType = "EXTRA_CREATE_DOC_TYPES";

        /// <summary>
        /// Intent Extra constant to pass list of allowed types to CreateDocument activity.
        /// </summary>
        public const string ExtraDocName = "EXTRA_CREATE_DOC_NAME";
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            if (this.context.PackageManager.CheckPermission(Manifest.Permission.WriteExternalStorage, this.context.PackageName) == Permission.Granted)
            {
                this.StartPicker();
            }
            else
        ...
        private void StartPicker()
        {
            ...
            string docType = Intent.GetStringExtra(ExtraDocType);
            string docName = Intent.GetStringExtra(ExtraDocName);

            intent.SetType(docType);
            intent.PutExtra(Intent.ExtraTitle, docName);
            intent.AddCategory(Intent.CategoryOpenable);
            try
            {
                this.StartActivityForResult(Intent.CreateChooser(intent, "Select Save Location"), 43);
            }
            catch (Exception ex)
            {
